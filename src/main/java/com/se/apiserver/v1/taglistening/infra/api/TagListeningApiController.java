...
public class TagListeningApiController {
  ...
  public TagListeningApiController(
      TagListeningCreateService tagListeningCreateService,
      TagListeningReadService tagListeningReadService,
      TagListeningDeleteService tagListeningDeleteService) {
    this.tagListeningCreateService = tagListeningCreateService;
    this.tagListeningReadService = tagListeningReadService;
    this.tagListeningDeleteService = tagListeningDeleteService;
  }
  ...
  public SuccessResponse<Long> create(
      @RequestBody @Validated TagListeningCreateDto.Request request) {
    return new SuccessResponse<>(HttpStatus.CREATED.value(), "등록에 성공했습니다",
  ...
  public SuccessResponse<TagListeningReadDto.Response> read(@PathVariable(value = "id") Long id) {
    return new SuccessResponse<>(HttpStatus.OK.value(), "조회에 성공했습니다",
        tagListeningReadService.read(id));
  }
  ...
  public SuccessResponse<List<TagListeningReadDto.Response>> readByAccountId(
      @PathVariable(value = "id") Long accountId) {
    return new SuccessResponse<>(HttpStatus.OK.value(), "조회에 성공했습니다",
        tagListeningReadService.readByAccountId(accountId));
  }
  ...
  public SuccessResponse<List<TagListeningReadDto.Response>> readMy() {
    return new SuccessResponse<>(HttpStatus.OK.value(), "조회에 성공했습니다",
        tagListeningReadService.readMy());
  }
  ...
  public PaginationResponse<List<TagListeningReadDto.Response>> readAll(
      @Validated PageRequest pageRequest) {
    Page<TagListeningReadDto.Response> page = tagListeningReadService
        .readAllByPaging(pageRequest.of());
    return new PaginationResponse<>(HttpStatus.OK, "조회에 성공했습니다", page.getContent(),
        page.getContent().size(), page.getTotalPages(), page.getPageable().getPageNumber(),
