...
@SpringBootTest
@Transactional
class AccountFindIdUseCaseTest {
    @Autowired
    AccountFindIdUseCase accountFindIdUseCase;
    @Autowired
    ...
    void 아이디_찾기_성공() {
        ...
        //when
        AccountFindIdByEmailDto.Response response = accountFindIdUseCase.readByEmail("test@test.com");
        //then
        Assertions.assertThat(response.getId()).isEqualTo(accountFindIdUseCase.hideLastTwoCharacter(account.getIdString()));
    }
    ...
    void 아이디_찾기_실패() {
        ...
        //when
        //then
        Assertions.assertThatThrownBy(() -> {accountFindIdUseCase.readByEmail("test@test.com");})
