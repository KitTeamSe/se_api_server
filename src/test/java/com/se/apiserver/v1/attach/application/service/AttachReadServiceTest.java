...
class AttachReadServiceTest {
    ...
    void 첨부파일_조회_성공() {
        ...
        initData();
        //when
        AttachReadDto.Response attachNoParentRes = attachReadService.read(attachNoParent.getAttachId());
        AttachReadDto.Response attachPostRes = attachReadService.read(attachPost.getAttachId());
        AttachReadDto.Response attachReplyRes = attachReadService.read(attachReply.getAttachId());
        //then
    ...
    void 첨부파일_전체_조회_성공() {
        ...
        initData();
        //when
        PageImpl page = attachReadService.readAllByPage(new PageRequest(1, 10, Sort.Direction.ASC).of());
        //then
    ...
    void 게시글별_첨부파일_조회_성공() {
        ...
        initData();
        //when
        List<AttachReadDto.Response> responses = attachReadService.readAllByPostId(post.getPostId());
        //then
    ...
    void 댓글별_첨부파일_조회_성공() {
        ...
        initData();
        //when
        List<AttachReadDto.Response> responses = attachReadService.readAllByReplyId(post.getPostId());
        //then
        Assertions.assertThat(responses.size()).isEqualTo(1);
    }
}